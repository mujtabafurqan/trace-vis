...
// getLatenciesLastHourBarChart("cartservice").then((data) => {
//   console.log("getLatenciesLastHourBarChart------------",data);
// });

export async function getTraces20(serviceName){

    const jaegerUrl = `http://localhost:8080/jaeger/ui/api/traces?limit=20&lookback=1h&service=cartservice`;
    try {
      const response = await axios.get(jaegerUrl);
      console.log("Traces!!",response.data);
      if (response.status === 200) {
